...
class AutoAiming:
    ...
    def callback(self, img):
        ...
        if SHOOT:
            self.controller.move_to(-np.pi/2, 0, PITCH_OFFSET, True)
            SHOOT = False

        if SHOOT and tar != None :
            self.shoot_target(tar, tar_bbox)

        if self.cumulated_angle >= np.pi / 2:
            print('=========================')
            print(f'steps: {len(self.vx)}')
            print(f'vx_mean: {np.mean(self.vx)}')
            print(f'vy_mean: {np.mean(self.vy)}')
            print(f'vx: {self.vx}')
            print(f'vy: {self.vy}')
        else:
    ...
    def trace_target(self, tar, labels=None):
        ...
        self.controller.move(STEP, 0, PITCH_OFFSET)

        self.vx.append(vx)
        self.vy.append(vy)

        self.cumulated_angle += STEP
