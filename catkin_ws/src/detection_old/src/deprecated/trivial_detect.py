...
IMG_W = 1280
IMG_H = 720
PITCH_OFFSET = 0.04 # for raising the head
MAX_YAW = 0.06
MAX_PITCH = 0.03
YAW_RANGE = 200
PITCH_RANGE = 150
...
class AutoAiming:
    def __init__(self):
        #ros.init_node('auto_aiming_trivial')
        ros.init_node('auto_aiming_test',argv=sys.argv,anonymous=False,disable_signals=True,log_level=ros.INFO)
        # the followiing line was not in original program
        sys.argv=ros.myargv(argv=sys.argv)
    ...
    def trace_target(self, tar, labels=None):
        ...
        self.controller.move(ax, -ay, PITCH_OFFSET, log=True)
        ...
        return [dx, dy], [pred_tar[0], pred_tar[1]], [ax, ay]
