...
std::vector<Armor> Detector::detect(const cv::Mat & input)
{
    ...
    // {
    //     ROS_INFO("pos : %f %d", light.length, light.color);
    // }
    
    armors_ = matchLights(lights_);

    ROS_INFO("count: %d",armors_.size());
...
void Detector::preprocessImage(const cv::Mat & rgb_img, cv::Mat & bin_img)
{
    cv::Mat gray_img;
...
std::vector<Light> Detector::findLights(const cv::Mat & rgb_img, const cv::Mat & binary_img)
{
    ...
    for (const auto & contour : contours)
    {
        ...
        if (isLight(light))
        {
            ...
            if (0 <= rect.x && 0 <= rect.width && rect.x + rect.width <= rgb_img.cols && 0 <= rect.y &&
...
std::vector<Armor> Detector::matchLights(const std::vector<Light> & lights)
{
    ...
    Match_id.resize(lights.size());
    Match_loss.resize(lights.size());
    // Loop all the pairing of lights
    for (int id_1 = 0; id_1 < lights.size(); id_1++)
    {
        int best_id = -1;
        double best_loss = 100000000000;
        for (auto id_2 = 0; id_2 < id_1; id_2++)
        {
            ...
            ROS_INFO("type: %s",ARMOR_TYPE_STR[static_cast<int>(type)].c_str());
        ...
        Match_id[id_1] = best_id;
        Match_loss[id_1] = best_loss;
    }
    ...
    ROS_INFO("light_num: %d  Armor_num: %d", lights.size(), armors.size());
    return armors;
}
...
ArmorType Detector::isArmor(const Light & light_1, const Light & light_2)
{
    ...
    // ROS_INFO("is_armor: %d , %d , %d", light_ratio_ok, center_distance_ok, z);
    // x
    // Judge armor type
    ArmorType type;
    if (is_armor)
