...
Tracker::Tracker(double max_match_distance, double max_match_yaw_diff) : tracker_state(LOST),
...
void Tracker::init(my_msgs::Armors armors_msg)
{
...
void Tracker::update(my_msgs::Armors armors_msg)
{
    // KF predict
    ...
    if (!armors_msg.armors.empty())
    {
        ...
        for (const auto & armor : armors_msg.armors)
        {
            ...
            if (armor.number == tracked_id)
            {
                ...
                ROS_INFO("pos:  (%.2f,%.2f,%.2f)", p.x, p.y, p.z);
        ...
        else
        {
            ...
            ROS_INFO("No matched armor found!");
        }
    }
