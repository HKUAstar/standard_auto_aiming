...
void YOLOv8::make_pipe(bool warmup)
{
	...
	for (auto& bindings : this->output_bindings)
	{
		...
		this->device_ptrs.push_back(d_ptr);		// Device(Cuda) memory for output.
		this->host_ptrs.push_back(h_ptr);		// Host(CPU) memory for output.
	}
...
void YOLOv8::infer()
{
	...
	int flag = this->context->enqueueV2(
	...
	for (int i = 0; i < this->num_outputs; i++)
	{
		...
		// debug(this->output_bindings[i].size);
